/*!
 * Script borrowed as is from Thom's APF quick questions repo
 * There is probably a better way to obtain the various scripts but...
 * this will certainly do the job for now
 * @todo improve!
 */
...
var connectionSpeed = (function () {
	...
	var imageAddr = "https://s3-ap-southeast-2.amazonaws.com/apf-quickquestions/31120037-5mb.jpg",
		downloadSize = 4995374;
	...
	var ShowProgressMessage = function(msg) {
	    ...
	    $('.speedConnection').html(actualHTML);
	    $('.formSpeedConnection').val(actualHTML);
	...
	var InitiateSpeedDetection = function() {
	...
	return {
		InitiateSpeedDetection: InitiateSpeedDetection
	}

})();
var userLocation = (function () {

	var ipAddress = $('[name="ip_address"]').attr('content');

	var country = function() {

		var promise = ajaxQuery();
		promise.done(function (data) {
			if(data.country_name != "") {
				var countryName = data.country_name;
			} else {
				var countryName = "Unavailable";
			}
			$('.techCountry').html(countryName);
			$('.formTechCountry').val(countryName);
		});
	};
...
var translations = (function () {
	...
	var $translationLinks = $('.translation-links a');

	var init = function() {
		// Bind click event to Translation bar
		$translationLinks.on('click', setLocaleByLink);
		// Get current locale
		var locale = getLocaleFromCookie('googtrans');
		$('a[data-locale="'+locale+'"]').addClass('active');
		// Change text direction
		setTextDirection(locale);
	};

	var getLocaleFromCookie = function(name) {
	    var value = "; " + document.cookie;
	    var parts = value.split("; " + name + "=");
	    if (parts.length == 2) {
	    	var googtrans = parts.pop().split(";").shift();
	    } else {
	    	var googtrans = '/en/en';
	    }
		var lang = googtrans.split("/");
		return lang[lang.length - 1];
	};

	var setLocaleByLink = function(e) {
		e.preventDefault();
		var lang = $(this).data('lang');
		var locale = $(this).data('locale');
		// Switch the language in the Google panel
		...
		// Update active class
		$translationLinks.removeClass('active');
		$('a[data-locale="'+locale+'"]').addClass('active');
	...
	var setTextDirection = function(locale) {
		if(locale == "ar") {
			$('body').attr('dir','rtl').removeClass().addClass('rtl');
		} else {
			$('body').attr('dir','ltr').removeClass().addClass('ltr');
		}
	};
...
$(document).ready(function() {
	...
	$('.form').parsley({
		...
		errorsWrapper: '<div class="form__feedback"></div>',
	...
	$('.techOs').html(techStack.operatingSystem());
    $('.techBrowser').html(techStack.browser());
    $('.techResolution').html(techStack.resolution());

    $('.formTechOs').val(techStack.operatingSystem());
    $('.formTechBrowser').val(techStack.browser());
    $('.formTechResolution').val(techStack.resolution());

	if (window.addEventListener) {
	    window.addEventListener('load', connectionSpeed.InitiateSpeedDetection(), false);
	} else if (window.attachEvent) {
	    window.attachEvent('onload', connectionSpeed.InitiateSpeedDetection());
	}
